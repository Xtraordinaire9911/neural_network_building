...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, embeddings_batch):
        ...
        print("self.positional_encodings_table.shape:", self.positional_encodings_table.shape)
        positional_encodings = self.positional_encodings_table[:embeddings_batch.shape[1], :]
        print("embeddings_batch.shape:", embeddings_batch.shape)
        print("positional_encodings.shape:", positional_encodings.shape)
...
if __name__ == "__main__":
    ...
    transformer = Transformer(
        model_dimension=1024 if use_big_transformer else 512,
        src_vocab_size=src_vocab_size,
        trg_vocab_size=trg_vocab_size,
        number_of_heads=16 if use_big_transformer else 8,
        number_of_layers=16 if use_big_transformer else 8,
        dropout_probability=0.3 if use_big_transformer else 0.1
    )
    """transformer = Transformer(
